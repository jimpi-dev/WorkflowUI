...
| Area               | Description                                                                                                                                                                                                  |
|--------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Import**         | Paste or upload ComfyUI workflow JSON; the app analyzes inputs/outputs and stores versioned workflow definitions. Idempotent import (same graph hash = no duplicate version).                                |
| **Metadata**       | Workflow and Apps-MetaData can be shared with each downloaded image, share the images to allow others to recreate the same image with the same workflow and apps and its parameters!                         |
| **Workflows**      | Stored definitions and versions with detected inputs/outputs; view and manage from the Workflows page.                                                                                                       |
| **Apps**           | Create *WorkflowApps* from any workflow version: custom title, form layout, default values. Open any app by its name.                                                                    |
...
For persistent data and more options (ComfyUI URL, media storage, etc.), see [docker/README.md](docker/README.md).
...
```bash
git clone <your-repo-url> WorkflowUI
cd WorkflowUI
```
...
## Documentation
- The application 'home' page does feature a blog style content, sharing updates and knowledge.
