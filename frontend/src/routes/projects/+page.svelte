<script lang="ts">
	...
	let colorFilter = $state<string>('all');
	...
	const distinctHeaderColors = $derived(
		[...new Set(data.projects.map((p) => p.header_color).filter((c): c is string => Boolean(c && String(c).trim())))].sort()
	);

	const colorFilterOptions = $derived.by(() => {
		const projects = data.projects;
		const noColorCount = projects.filter((p) => !p.header_color).length;
		const byColor = new Map<string, number>();
		for (const p of projects) {
			const c = p.header_color;
			if (c && String(c).trim()) byColor.set(c, (byColor.get(c) ?? 0) + 1);
		}
	...
	function matchesColorFilter(p: ProjectSummary): boolean {
		if (colorFilter === 'all') return true;
		if (colorFilter === '') return !p.header_color;
		return (p.header_color ?? '') === colorFilter;
	}

	const filtered = $derived(
		data.projects.filter((p) => matchesSearch(p) && matchesColorFilter(p))
	);
			...
			<div class="color-filter" role="group" aria-label="Filter by color">
				{#each colorFilterOptions as opt}
					<button
						...
						aria-label="{opt.swatch ? `Color ${opt.value} (${opt.count} projects)` : `${opt.label} (${opt.count} projects)`}"
					>
						{#if opt.swatch}
							<span class="color-filter-swatch" style="background-color: {opt.swatch}" aria-hidden="true"></span>
						{:else}
							...
							{#if p.tags?.length}
								<div class="project-tags">
									...
									{#if (p.tags?.length ?? 0) > 4}
										<span class="tag tag-more">+{(p.tags?.length ?? 0) - 4}</span>
									{/if}
								</div>
							{/if}
						{/if}
