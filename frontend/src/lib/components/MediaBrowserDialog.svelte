<script lang="ts">
    ...
    const displayedItems = $derived.by(() => {
        if (!filterFavoritesOnly) return items;
        return items.filter((item) => item.source === 'generation' && item.is_favorite === true);
    });
                    ...
                    <div class="media-grid">
                        {#each displayedItems as item, idx (itemKey(item, idx))}
                            ...
                            >
                                ...
                                {/if}
                                <img src={imageUrlFor(item)} alt="" loading="lazy" referrerpolicy="no-referrer" onload={(e) => onThumbLoad(item, idx, e)} />
                                <div class="meta">
