<script lang="ts">
	...
	function startResize(e: MouseEvent) {
		...
		function onMove(ev: MouseEvent) {
			...
			queuePanelWidth = clampPanelWidth(startW + delta);
		}
		function onUp() {
			resizing = false;
	...
	function onWindowResize() {
		queuePanelWidth = clampPanelWidth(queuePanelWidth);
	}

	onMount(() => {
		collapseAllByDefault = getQueueGroupsCollapsedCookie();
		queuePanelWidth = clampPanelWidth(getQueuePanelWidthCookie());
	...
	function handleDragOver(e: DragEvent, runId: string) {
		// When dragging a group, only headers are valid drop targets; list items do not highlight.
	...
	async function handleDrop(e: DragEvent, targetRunId: string, isGroupHeader = false) {
		...
		// When dragging a group, only drops on run headers are accepted; drops on run lines are ignored.
...
{#if !$queuePanelOpen}
	<!-- Compact badge on right edge when panel is closed -->
	<button
		type="button"
		class="queue-badge"
...
{#if $queuePanelOpen}
	<!-- Right-side panel: no backdrop so user can navigate the app while queue stays open -->
	<div
		class="queue-panel"
		class:resizing
		role="dialog"
		...
		aria-modal="false"
		style="width: {clampPanelWidth(queuePanelWidth)}px;"
		onkeydown={handleKeydown}
	>
		<!-- Resize handle: drag to change panel width (max 33% of viewport) -->
		<div
		...
		></div>
		<!-- Collapse tab on left edge of panel: click to slide panel back -->
		<button
			type="button"
...
<style>
	/* Badge: fixed on right edge, always visible; click toggles panel. Stays visible when navigating. */
	.queue-badge {
		position: fixed;
		right: 0;
		top: 50%;
	...
	/* Right-side panel: full height, resizable width (max 33% viewport) */
	.queue-panel {
		position: fixed;
		top: 0;
		right: 0;
		...
		display: flex;
		flex-direction: row;
		animation: queue-slide-in-right 0.25s ease-out;
	}
	...
	.queue-group-remove,
	.queue-group-remove .queue-group-remove-icon {
		color: color-mix(in srgb, var(--text) 55%, transparent);
	}
	...
	.queue-group-remove:hover .queue-group-remove-icon {
		color: var(--warning);
	}
	...
	.queue-group.group-dragging .queue-group-header,
	.queue-group.group-dragging .queue-list {
		opacity: 1;
	}
	/* When dragging a group, only headers are valid drop targets; run lines are greyed out. */
	...
	@keyframes queue-slide-in-right {
